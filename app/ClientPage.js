...
const SpecialDrinkIcon = ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><polyline points="15 18 9 12 15 6"></polyline></svg>;
...
const MetalIcon = ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" className={className}><rect width="18" height="18" x="3" y="3" rx="2"/><path d="M7 7h.01"/><path d="M17 7h.01"/><path d="M7 17h.01"/><path d="M17 17h.01"/></svg>;
const WoodIcon = ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" className={className}><path d="m17 14 3 3.3a1 1 0 0 1-.7 1.7H4.7a1 1 0 0 1-.7-1.7L7 14h-.3a1 1 0 0 1-.7-1.7L9 9h-.2A1 1 0 0 1 8 7.3L12 3l4 4.3a1 1 0 0 1-.8 1.7H15l3 3.3a1 1 0 0 1-.7 1.7H17Z"/><path d="M12 22v-3"/></svg>;
const PlasticIcon = ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" className={className}><path d="M8 2h8"/><path d="M9 2v2.789a4 4 0 0 1-.672 2.219l-.656.984A4 4 0 0 0 7 10.212V20a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2v-9.789a4 4 0 0 0-.672-2.219l-.656-.984A4 4 0 0 1 15 4.788V2"/><path d="M7 15a6.472 6.472 0 0 1 5 0 6.47 6.47 0 0 0 5 0"/></svg>;
const MenuIcon = ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" className={className}><path d="M3 5h18"/><path d="M3 12h18"/><path d="M3 19h18"/></svg>;

const GoDrinkLogo = () => (
    <div className="flex items-center space-x-2">
<svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-recycle-icon lucide-recycle"><path d="M7 19H4.815a1.83 1.83 0 0 1-1.57-.881 1.785 1.785 0 0 1-.004-1.784L7.196 9.5"/><path d="M11 19h8.203a1.83 1.83 0 0 0 1.556-.89 1.784 1.784 0 0 0 0-1.775l-1.226-2.12"/><path d="m14 16-3 3 3 3"/><path d="M8.293 13.596 7.196 9.5 3.1 10.598"/><path d="m9.344 5.811 1.093-1.892A1.83 1.83 0 0 1 11.985 3a1.784 1.784 0 0 1 1.546.888l3.943 6.843"/><path d="m13.378 9.633 4.096 1.098 1.097-4.096"/></svg>
<span className="text-2xl font-bold tracking-wider">GoGreen</span>
    </div>
);

const categories = [
    { name: 'Plastic', thaiName: 'พลาสติก', icon: PlasticIcon, color: 'bg-amber-300' },
    { name: 'Wood', thaiName: 'ไม้', icon: WoodIcon, color: 'bg-yellow-500' },
    { name: 'Glass', thaiName: 'แก้ว', icon: TeaIcon, color: 'bg-blue-400' },
    { name: 'Metal', thaiName: 'โลหะ', icon: MetalIcon, color: 'bg-neutral-400' },
];
...
//const Header = () => (<header className="sticky top-0 z-20 bg-gradient-to-r from-lime-500 to-emerald-500 text-white p-4 flex justify-between items-center shadow-lg"><GoDrinkLogo /><div className="flex items-center space-x-4"><SearchIcon className="h-6 w-6 cursor-pointer hover:opacity-80 transition-opacity" /><UserIcon className="h-6 w-6 cursor-pointer hover:opacity-80 transition-opacity" /></div></header>);
const Header = ({ onLogoClick }) => (
    <header className="sticky top-0 z-20 bg-gradient-to-r from-lime-500 to-emerald-500 text-white p-4 flex justify-between items-center shadow-lg">
...
const CartView = ({ cart, onUpdateCart, onBack }) => {
    ...
    return (<div className="p-4 bg-gray-50 min-h-screen">
        ...
        <span>ยอดรวมทั้งหมด</span><span>฿{total.toFixed(2)}</span></div></div>
        {/* แก้ไขตรงนี้ */}
        <a href="https://img5.pic.in.th/file/secure-sv1/maxresdefaultc2026c2b5cf29a62.jpg" 
           target="_blank" 
           rel="noopener noreferrer" 
           className="w-full mt-6 bg-purple-600 text-white font-bold py-3 rounded-xl shadow-lg hover:bg-purple-700 transition-colors text-lg text-center inline-block"
        >
          ชำระเงิน
        </a>
        {/* สิ้นสุดการแก้ไข */}
                    ...
                    <div className="p-4 sm:p-6">
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-4">
                               <h3 className="text-2xl font-bold text-gray-800">{selectedCategory ? `${selectedCategory}` : 'เมนูแนะนำ'}</h3>
                               {selectedCategory && (
